...
/* Get from storare section the html code that will be store in home_http_site
 * variable */
void get_index_site() {
  // Initialize SPIFFS partition
  esp_vfs_spiffs_conf_t spiffs = {.base_path = "/storage",
  ...
  if (ret != ESP_OK) {
    if (ret == ESP_FAIL) {
      ...
      ESP_LOGE("Http server", "Failed to initialize SPIFFS (%s)",
               esp_err_to_name(ret));
    }
    return;
  }
  ...
  if (ret != ESP_OK) {
    ESP_LOGE("Http server", "Failed to get SPIFFS partition information (%s)",
             esp_err_to_name(ret));
  } else {
...
esp_err_t home_post_handler(httpd_req_t *req) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE("Http server", "Error (%s) send response (url: /, type: POST)",
             esp_err_to_name(err));
  } else {
...
esp_err_t mac_post_handler(httpd_req_t *req) {
  ...
  if (err != ESP_OK) {
    ...
    if (recv_size != 12) {
      ...
      if (err != ESP_OK) {
        ESP_LOGE("Http server",
                 "Error (%s) send response (url: /mac, type: POST)",
                 esp_err_to_name(err));
      } else {
  ...
  if (err != ESP_OK) {
    ESP_LOGE("Http server", "Error (%s) send response (url: /mac, type: POST)",
             esp_err_to_name(err));
  } else {
    ESP_LOGI("Http server", "Http response sent (url: /mac, type: POST)");
  }
...
void update_mac_address(nvs_handle_t nvs_handle) {
  ...
  char *possible_mac = malloc(string_size);
  ...
  case ESP_OK:
    ...
    ESP_LOGI("Http server", "NVS get destination MAC address: %.12s",
             dest_mac_addr);
    break;
...
void http_server_start(nvs_handle_t nvs_handle) {
  ...
  httpd_uri_t home_get_uri = {.uri = "/",
                              .method = HTTP_GET,
                              .handler = home_get_handler,
                              .user_ctx = NULL};
  httpd_uri_t home_post_uri = {.uri = "/",
                               .method = HTTP_POST,
                               .handler = home_post_handler,
                               .user_ctx = NULL};

  httpd_uri_t mac_get_uri = {.uri = "/mac",
                             .method = HTTP_GET,
                             .handler = mac_get_handler,
                             .user_ctx = NULL};
  httpd_uri_t mac_post_uri = {.uri = "/mac",
                              .method = HTTP_POST,
                              .handler = mac_post_handler,
                              .user_ctx = NULL};

  httpd_uri_t mac_options_uri = {.uri = "/*",
                                 .method = HTTP_OPTIONS,
