...
static esp_err_t write_cb(const esp_rmaker_device_t *device,
  ...
  if (strcmp(esp_rmaker_param_get_name(param), ESP_RMAKER_DEF_POWER_NAME) ==
      0) {
...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == RMAKER_EVENT) {
    ...
    case RMAKER_OTA_EVENT_REQ_FOR_REBOOT:
      ESP_LOGI(TAG, "Firmware image downloaded. Please reboot your device to "
                    "apply the upgrade.");
      break;
    default:
...
void start_rainmaker(nvs_handle_t nvs_handle) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(
      RMAKER_COMMON_EVENT, ESP_EVENT_ANY_ID, &event_handler, NULL));
  ...
  esp_rmaker_node_t *node =
      esp_rmaker_node_init(&rainmaker_cfg, "ESP RainMaker Device", "Switch");
  if (!node) {
  ...
  switch_device =
      esp_rmaker_device_create("Computer", ESP_RMAKER_DEVICE_SWITCH, NULL);
  ...
  esp_rmaker_device_add_param(
      switch_device,
      esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Computer"));
  ...
  esp_rmaker_param_add_ui_type(power_param, ESP_RMAKER_UI_TRIGGER);

  /* Assign the power parameter as the primary, so that it can be controlled
   * from the home screen of the phone apps */
   ...
   * timezone from the phone apps for scheduling to work correctly.
   */
  ...
  esp_err_t err = app_wifi_set_custom_mfg_data(MGF_DATA_DEVICE_TYPE_SWITCH,
